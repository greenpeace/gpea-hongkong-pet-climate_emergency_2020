        ...
        <div class="form-row">
          <ValidationProvider rules="required|email" v-slot="{ classes, errors }">
            ...
            <span
              ...
              @click="replaceEmailWithSuggestion"
              >您想輸入的是 <strong>{{ emailSuggestion }}</strong> 嗎？</span
            >
          </ValidationProvider>
        </div>
          ...
          <ValidationProvider rules="required" v-slot="{ classes, errors }">
            ...
            <div class="relative">
              <select name="Birthdate" v-model="Birthdate" :class="['form-input', classes]">
                ...
                <option v-for="year in years" v-bind:value="year + '-01-01'" v-bind:key="year">
                  {{ year }}
                </option>
              </select>
            </div>
        ...
        <div class="form-row py-2">
          <ValidationProvider rules="required" v-slot="{ classes, errors }">
            <label class="flex">
              ...
              >
                <input class="hidden" checked type="checkbox" v-model="OptIn" />
                <svg
